...
console.log("rates in faucet ", ckBTCUsdRate,
  ckETHUsdRate,
  ckUSDCUsdRate,
  ckICPUsdRate,
  ckUSDTUsdRate,)
...
console.log("faucet Amount",faucetBTC, faucetETH, faucetUSDC, faucetICP, faucetUSDT)
  useEffect(() => {
  ...
  //  if(backendActor){
  //   const result =  backendActor.update_reserves_price();
  //   console.log("result in update reserve",result)
  //  }
   console.log("Available amount for", asset, FaucetLimit[asset],FaucetUsage[asset])
    if (exchangeRate) {
  ...
  const handleFaucet = async (asset) => {
    setLoading(true); // Start loading when the faucet request begins
    try {
      if (backendActor) {
        console.log("Amount in faucet:", amount);
        const numericAmount = parseFloat(amount.replace(/,/g, "")); // Parse the amount
        ...
        const natAmount = Math.round(numericAmount * Math.pow(10, 8) ); // Convert to nat (integer representation)
        console.log("Rate in faucet popup", ckBTCUsdRate, ckETHUsdRate, ckUSDCUsdRate, ckICPUsdRate, ckUSDTUsdRate);
  
        
  
        // Calculate usdAmount based on the asset
        console.log("natAmount", natAmount);
       
       
  
        // Get the remaining available amount (FaucetLimit - FaucetUsage)
        const availableAmount = FaucetLimit[asset] - FaucetUsage[asset];
        console.log("Available amount for", asset, FaucetLimit[asset],FaucetUsage[asset],availableAmount);
  
        // If the usdAmount exceeds the remaining available amount, show error
        if ( numericAmount > availableAmount) {
          ...
          setLoading(false);
          return; // Exit the function if the amount exceeds the available limit
        }
  
        // Proceed with the faucet request
                ...
                <div className="w-full flex items-center justify-between bg-gray-100 hover:bg-gray-300 p-3 rounded-md dark:bg-[#1D1B40] dark:text-darkText">
                  ...
                  <div className="w-9/12 flex flex-col items-end">
                    ...
                    >
                      ...
                      {"   "} $
                      {(
                        (FaucetLimit[asset]/1e8) - (FaucetUsage[asset]/1e8)
                      ).toLocaleString()}{" "}
                      Max
                    </p>
                  </div>
