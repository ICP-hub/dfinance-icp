...
// }
pub fn percent_mul(value: u128, percentage: u128) -> Result<u128, String> {
    ...
    if percentage == 0 || value > (u128::MAX - HALF_PERCENTAGE_FACTOR) / percentage {
        return Err("Overflow in percentage multiplication".to_string());
    }
...
/// `percentage`: The percentage of the value to be calculated
/// Returns the result of value / (percentage / 100.00), rounded.
pub fn percent_div(value: u64, percentage: u64) -> Result<u64,String> {
    ...
    if percentage == 0 || value > (u64::MAX - percentage / 2) / PERCENTAGE_FACTOR {
        return Err("Overflow in percentage division".to_string());
    }

    Ok((value * PERCENTAGE_FACTOR + percentage / 2) / percentage)
}
