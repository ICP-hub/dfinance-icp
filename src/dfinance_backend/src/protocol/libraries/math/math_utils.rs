...
use crate::constants::interest_variables::constants::{HUNDRED_UNITS, SIX_UNITS, TWO_UNITS};
...
pub fn calculate_linear_interest(rate_input: Nat, last_update_timestamp: u64) -> Nat {
    let rate = rate_input/Nat::from(HUNDRED_UNITS);
...
pub fn calculate_compounded_interest(
    ...
    let second_term = (exp as u128 * exp_minus_one as u128 * base_power_two) / Nat::from(TWO_UNITS);
    ic_cdk::println!("Second term: {}", second_term);
    
    let third_term =
        (exp as u128 * exp_minus_one as u128 * exp_minus_two as u128 * base_power_three) / Nat::from(SIX_UNITS);
