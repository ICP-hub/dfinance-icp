          ...
          <div key={section} className="mb-8">
           <div className="p-4 mb-8  bg-gradient-to-r from-[#4659CF]/40 via-[#D379AB]/40 to-[#FCBD78]/40 dark:bg-bottom-left-to-top-right-gradient  text-[#2A1F9D] dark:text-darkText rounded-lg  flex justify-between items-center">
              <div className="flex items-center space-x-4">
                <h3 className="text-sm font-semibold  ss1:font-[500]">{section}</h3>
                ...
                {isTableView ? (
                  <div>
                    ...
                    <div className="grid grid-cols-5 gap-10 font-[500] text-sm text-[#233D63] dark:text-darkTextSecondary">
                      <div className="flex items-center  text-center py-2">Asset</div>
                      <div className="flex items-center text-center py-2">Amount</div>
                      <div className="flex items-center text-center py-2">Asset Points</div>
                      <div className="flex items-center text-center py-2">Points Accrued</div>
                    ...
                    <div className="">
                      {currentTransactions.length > 0 ? (
                        currentTransactions.map((tx, index) => (
                          <div
                            key={index}
                            className={`grid grid-cols-5 gap-10 py-4 text-[#2A1F9D] text-sm font-bold dark:text-darkText hover:bg-[#ddf5ff8f] dark:hover:bg-[#8782d8] rounded-lg  ${index !== currentTransactions - 1 ? "gradient-line-bottom" : ""}`}
                          >
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-10 py-4 font-[500] text-sm text-[#233D63] dark:text-darkTextSecondary mb-2">
                      <div className=" flex items-center text-center ">Asset</div>
                      <div className="flex items-center text-center text-nowrap -ml-4">Points Accured </div>
                     
                      <div className="flex items-center text-center ">More</div>
                    </div>
                    {currentTransactions.length > 0 ? (
                      currentTransactions.map((tx, index) => (
                        <div
                    key={index}
                    className={`grid grid-cols-3 gap-10 py-4  text-[#2A1F9D] text-sm font-medium dark:text-darkText  rounded-lg  ${index !== currentTransactions - 1 ? "gradient-line-bottom" : ""}`}
                  >
                          <div className="flex items-center text-center space-x-2 ">
                            <img
                              src={tx.imageUrl}
