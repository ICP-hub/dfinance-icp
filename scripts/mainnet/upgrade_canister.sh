...
dfx canister uninstall-code dtoken --ic
dfx canister uninstall-code debttoken --ic
dfx canister uninstall-code token_ledger --ic
...
dfx canister install dtoken --mode=upgrade --argument "(variant {Init =
record {
...
export CYCLE_FOR_ARCHIVE_CREATION=10000000000000
export FEATURE_FLAGS=true
dfx canister install token_ledger --mode=upgrade --argument "(variant {Init =
record {
     token_symbol = \"${TOKEN_SYMBOL}\";
     token_name = \"${TOKEN_NAME}\";
     minting_account = record { owner = principal \"${MINTER}\" };
...
dfx canister uninstall-code dfinance_backend --ic
dfx canister install dfinance_backend --ic --mode=install --argument "(principal \"$admin_principal\" )"
