...
pub async fn execute_supply(
    ...
    if params.asset.is_empty() || params.amount <= Nat::from(0u128) {
    ...
    let total_supplies = reserve_data.asset_supply.clone() + params.amount.clone();
    let total_borrow = reserve_data.asset_borrow.clone();
...
pub async fn execute_withdraw(
    ...
    reserve_data.total_supply = (reserve_data.total_supply - usd_amount).max(Nat::from(0u128));
    ic_cdk::println!(
