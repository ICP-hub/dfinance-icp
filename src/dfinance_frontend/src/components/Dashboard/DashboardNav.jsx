...
const DashboardNav = () => {
  ...
  return (
    <div className="w-full">
      ...
      <div className="w-full flex py-2 items-center gap-2">
        ...
        <div id="info_text" className="flex px-6 ml-3">
          {isWalletCreated ? (
            <div className="flex items-center text-[#2A1F9D] font-semibold gap-6">
              {
                WALLET_DETAILS_TABS.map((data, index) => (
                  <button key={index} className="relative" onMouseEnter={() => handleHoverIn(index)} onMouseLeave={handleHoverOut}>
                    {data.title}
                    <hr className={`ease-in-out duration-500 bg-[#8CC0D7] h-[2px] ${hoverEle.index === index && hoverEle.isHover ? "w-full" : "w-1/5"}`}/>
                    {hoverEle.index === index && hoverEle.isHover && <span className="animate-fade absolute top-full left-0 font-light py-2">${data.count}M</span>}
                  </button>
                ))
              }
            </div>
