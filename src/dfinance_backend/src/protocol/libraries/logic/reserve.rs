...
pub async fn burn_scaled(
    ...
    let mut adjusted_amount = if amount.clone() % index.clone() != Nat::from(0u128) &&  amount.clone() < Nat::from(1000u128) {
...
pub async fn mint_scaled(
    ...
    if amount.clone() > adjusted_amount.clone() && amount.clone()-adjusted_amount.clone() < Nat::from(1000u128){
